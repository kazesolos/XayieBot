...
# ------------- CONFIG -------------
# These will come from Render "Environment" tab
...
async def handle_ai_interaction(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str):
    ...
    user_data = context.user_data  # per-user dict in PTB 20+
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "/whoami – Tell you who you are (owner + saved names)\n"
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = msg.text or ""
    prompt = prompt.replace(f"@{BOT_USERNAME}", "").replace(
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_warns >= 3:
        ...
        await msg.reply_text(
            f" {target_user.mention_html()} has been warned. "
